...
excerpt: "Exploring the cryptographic primitives and architecture patterns behind zero-knowledge systems, and how we implemented them in a production secrets manager."
---
...
We use **Argon2id** to derive a master key from the user's password:
...
The salt is stored on the server (not secret), but the derived key never leaves the client.
...
What happens when the user changes their password? The solution: **two-layer encryption**.
...
We use **SRP (Secure Remote Password)** — a protocol where the server verifies a proof that the client knows the password without the password ever being sent:
...
**Key memory management** — Master keys in JavaScript `CryptoKey` objects live in JS heap until GC. For highly sensitive applications, consider WebAssembly with manual memory management.

**Multi-device sync** — Device A encrypts the Vault Key with Device B's public key and sends it through the server. The server sees an encrypted blob; Device B decrypts with its private key.

**Recovery codes** — If the user loses their password, they're locked out forever (that's the point). We generate BIP-39 mnemonic recovery codes during registration.

**Search over encrypted data** — You can't do server-side search. We use client-side filtering: fetch all encrypted names, decrypt client-side, filter, then fetch matching secrets.

## Is It Worth It?

For a secrets manager? Absolutely. For a general CRUD app? Probably not — the UX compromises are significant. The implementation complexity is real, but for applications where trust is the product, ZK architecture is the only honest choice.
